...
class AuthServices {
  ...
  Future<TokenModel> generateToken() async {
    ...
    var header = {
      ...
      'Authorization':
          'Basic ZWRhMzVhZjAtZjRlMC00MDQ2LTg0YmUtNjlmOTNhNTQ1ZGJhOkxySXpZRHRlU1haTDdIalRjLTBXU25fbGo0M2VwZFAwdjZrS0NRRnhlMVRyQ0R3bDBhSGpaRmo5VzRaQ2RUelczYnJKbG9Lcm9vWTE4VU9EZW5VcVlR',
  ...
  //     print("The order value is" +
  //         mainScreenController.orderValue.value.toString());
  ...
  Future<GroceryModel> getGroceryList() async {
    ...
    /// Url
    String url = UrlSchemes.baseUrl + UrlSchemes.grocery;
    ...
    var header = {
      'Accept': 'application/json',
      'WM_SEC.ACCESS_TOKEN': generatedToken,
      'WM_QOS.CORRELATION_ID': 'bbb24ee7-efc1-4f93-851a-8410f6cf46e3',
      'WM_SVC.NAME': 'Walmart Marketplace',
      'sku': 'abc123',
    };
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      // mainScreenController.itemList = groceryModelFromJson(response.body).items!;
      groceryModelFromJson(response.body);
    }
    if (response.statusCode == 400) {
      // Prompts.showError("Oops",errorModelFromJson(response.body).error);
      // return errorModelFromJson(response.body);
